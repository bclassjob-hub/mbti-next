...
document.getElementById('app').innerHTML=`<div class="wrap"><div class="brand"><div class="logo">M</div>MBTI NEXT</div><div class="top"><button class="back" onclick="home()">← 16유형</button><span class="pill">${t} · ${g}</span></div><div class="panel"><div class="eyebrow">MY TYPE</div><div class="tt">${t}</div><div class="tag">${E(d.tagline)}</div><div class="summary">${E(d.detail.summary)}</div>${detailBox(d.detail)}</div><div class="tabs"><button class="${state.tab==='celeb'?'active':''}" onclick="setTab('celeb')">유명인</button><button class="${state.tab==='recommend'?'active':''}" onclick="setTab('recommend')">추천 활동</button><button class="${state.tab==='challenge'?'active':''}" onclick="setTab('challenge')">도전 활동</button></div>${body}<div class="signatureFooter">Made by RanCho · 2026</div></div>`}
home();