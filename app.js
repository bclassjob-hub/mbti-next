...
const LEGAL=`<div class="legalNotice">교육용 비공식 콘텐츠입니다. 공식 MBTI® 검사 또는 관련 기관의 공식 서비스가 아닙니다.<br>MBTI®는 Myers &amp; Briggs Foundation의 상표 또는 등록상표입니다.</div>`;
let ALL=null;
...
async function load(t){
  ...
  if(t==='ENFP'){
    const r=await fetch('data/types/enfp.json?v=20260915-easter5',{cache:'no-store'});
    if(!r.ok)throw Error('ENFP 데이터를 불러오지 못했습니다.');
    const data=await r.json();
    const me=(data.celebrities||[]).find(c=>c.easter_egg||c.status==='본인 확인');
    if(me){
      if(me.intro_long)me.intro_long=me.intro_long.replaceAll('MBTI NEXT','TYPE NEXT');
      if(Array.isArray(me.signature))me.signature=me.signature.map(x=>String(x).replaceAll('MBTI NEXT','TYPE NEXT'));
      if(Array.isArray(me.achievement))me.achievement=me.achievement.map(x=>String(x).replaceAll('MBTI NEXT','TYPE NEXT'));
    }
    return cache[t]=data;
  }
